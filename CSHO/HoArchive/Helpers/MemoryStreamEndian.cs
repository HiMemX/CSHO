...
namespace HoArchive{
    ...
    public class MemoryStreamEndian : MemoryStream{
        ...
        public MemoryStreamEndian NewStreamFromHere()
        {
            uint addr = (uint)Position;
            MemoryStreamEndian output = new MemoryStreamEndian(ReadBytes((int)(Length - Position)), endianness);
            Position = addr;
            return output;
        }
        
        public (float, float) ReadPQS(int scalingfactor, QuantizationType type = QuantizationType.SHORT)
        {
            float scalar = 1.0f / (float)Math.Pow(2, scalingfactor);

            switch (type)
            {
                case QuantizationType.FLOAT:
                    return (ReadFloat32E(), ReadFloat32E());

                case QuantizationType.SHORT:
                    return (scalar * (float)ReadInt16E(), scalar * (float)ReadInt16E());

                case QuantizationType.BYTE:
                    return (scalar * (float)ReadSByte(), scalar * (float)ReadSByte());
