...
namespace HoArchive{
    public class ParcelDebug : ParcelBase{
        ...
        public void AddEntry(ulong id, string name){
            NameTableEntries.Add(new NameTableEntry(id, name));
        }

        public string GetName(ulong id){
            foreach(NameTableEntry entry in NameTableEntries){
                if(entry.uidAsset == id) return entry.name;
            }
            return "";
        }
        ...
        public void Update(uint Align, uint sectorSize, bool Reversed){
            for(int i=0; i<NameTableEntries.Count; i++){
                if(!NameTableEntries[i].delete){
                    continue;
                }

                NameTableEntries.RemoveAt(i);
                i = 0;
            }
