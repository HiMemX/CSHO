...
class Table : ParcelBase{ // Table is a type of Parcel (SECT, P, PD, PTEX, PFST). It will have the same methodes as normal parcels to allow for a complex tree (It also saves me some code).
    ...
    public void Update(uint Align, uint sectorSize, bool Reversed){
        ...
        for (int parcel=0; parcel<Parcels.Count; parcel++){
            if (TableEntries[parcel].sectionType != "PD  " && TableEntries[parcel].sectionType != "SECT"){ // This weird stuff cause .hoes don't like being reversed where they shouldn't be
        ...
        TableHeader.stringTableSize = (uint)(StringTableLength + StringTable.DomainString.Length + 0x01);
    ...
    public void updateSectors(uint sectorSize, uint startSector){
        ...
        foreach (ParcelBase parcel in Parcels){
            parcel.updateSectors(sectorSize, currSector);
        }
    ...
    public uint getSize(List<SliceMeta> meta){
        ...
        uint size = MathTools.RoundUpTo((uint)(TableEntries.Count * 0x40 + 0x20 + StringTableLength + StringTable.DomainString.Length + 0x01), 0x20) + MetaTableLength;
        return size;
    }
